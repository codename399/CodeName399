...
REM ============================================================================
REM CodeName399 Windows Server Setup v16
REM IMPORTANT:
REM   - Run through the supplied launcher.
REM   - Build/deploy flags are FALSE by default.
REM   - Cloudflare tunnel is token-based and uses a token file.
REM   - No Cloudflare token is printed.
REM ============================================================================

set "SETUP_LOG=%TEMP%\CodeName399-ServerSetup.log"
if exist "%SETUP_LOG%" del /q "%SETUP_LOG%" >nul 2>&1

net session >nul 2>&1
if errorlevel 1 (
    echo [ERROR] Administrator privileges are required.
    echo Run Run-CodeName399-Setup-v16.cmd.
    pause
    exit /b 1
)
...
>>"%SETUP_LOG%" echo ============================================================
>>"%SETUP_LOG%" echo CodeName399 setup started %DATE% %TIME%
>>"%SETUP_LOG%" echo ============================================================

REM ============================================================================
REM FIXED PATHS
REM ============================================================================
set "BASE_PATH=C:\Users\gauda\source\repos\codename399"
set "API_PATH=C:\Users\gauda\source\repos\codename399\CodeName399.API"
set "SLN_PATH=C:\Users\gauda\source\repos\codename399\CodeName399.API\CodeName399.sln"
set "ANGULAR_PATH=C:\Users\gauda\source\repos\codename399\CodeName399"
...
set "BACKUP_PATH=%SERVER_ROOT%\Backups"
set "LOG_PATH=%SERVER_ROOT%\Logs"
...
REM New rotated token supplied by the user.
REM DO NOT ECHO THIS VALUE.
set "CLOUDFLARE_TUNNEL_TOKEN=eyJhIjoiMDE5M2VhZDgxMTQxMDBlMjQ0N2U0NDdmZTBjNWIxNDYiLCJ0IjoiNDk0YmIyOWYtZmI1NS00YzJhLWFlMzItMjdlZGVmNjQzMTFkIiwicyI6IllqTTRNREZrTURrdE5EZ3hOQzAwWldKbUxXSTVNRE10WVRaak9UZ3lOelJrWmpKbCJ9"

REM ============================================================================
REM FLAGS
REM ============================================================================
set "INSTALL_DOTNET=true"
set "INSTALL_NODE=true"
set "INSTALL_NGINX=true"
set "INSTALL_CLOUDFLARED=true"

REM API build/deploy flags - FALSE by default.
set "API_BUILD=false"
set "API_DEPLOY=false"

REM Individual project deployment flags - FALSE by default.
...
if not exist "%SLN_PATH%" (
    echo [ERROR] API solution missing: %SLN_PATH%
    goto :SetupFail
)
if not exist "%ANGULAR_PATH%\angular.json" (
    echo [ERROR] Angular project missing: %ANGULAR_PATH%\angular.json
    goto :SetupFail
)
if not exist "%ANGULAR_PATH%\package.json" (
    echo [ERROR] package.json missing: %ANGULAR_PATH%\package.json
    goto :SetupFail
)
...
call :EnsureDir "%SERVER_ROOT%" || goto :SetupFail
call :EnsureDir "%GATEWAY_PATH%" || goto :SetupFail
call :EnsureDir "%AUTH_PATH%" || goto :SetupFail
call :EnsureDir "%GAMESTASH_PATH%" || goto :SetupFail
call :EnsureDir "%DEBTMANAGER_PATH%" || goto :SetupFail
call :EnsureDir "%KITE_PATH%" || goto :SetupFail
call :EnsureDir "%UI_PATH%" || goto :SetupFail
call :EnsureDir "%BACKUP_PATH%" || goto :SetupFail
call :EnsureDir "%LOG_PATH%" || goto :SetupFail
...
if errorlevel 1 (
    if /I "%INSTALL_DOTNET%"=="true" (
        where winget >nul 2>&1
        if errorlevel 1 (
            echo [ERROR] dotnet missing and winget unavailable.
            goto :SetupFail
        )
...
where node >nul 2>&1
if errorlevel 1 (
    if /I "%INSTALL_NODE%"=="true" (
        where winget >nul 2>&1
        if errorlevel 1 (
            echo [ERROR] Node.js missing and winget unavailable.
            goto :SetupFail
        )
...
call :EnsureDir "%CONFIG_BACKUP%" || goto :SetupFail
...
if /I "%API_DEPLOY%"=="true" (
    if /I "%GATEWAY_DEPLOY%"=="true" call :DeployProject "%API_PATH%\CodeName399.Gateway\CodeName399.Gateway.csproj" "%GATEWAY_PATH%" "%GATEWAY_SERVICE%" "%GATEWAY_PORT%" || goto :SetupFail
    if /I "%AUTH_DEPLOY%"=="true" call :DeployProject "%API_PATH%\CodeName399.Auth.API\CodeName399.Auth.API.csproj" "%AUTH_PATH%" "%AUTH_SERVICE%" "%AUTH_PORT%" || goto :SetupFail
    if /I "%GAMESTASH_DEPLOY%"=="true" call :DeployProject "%API_PATH%\CodeName399.GameStash.API\CodeName399.GameStash.API.csproj" "%GAMESTASH_PATH%" "%GAMESTASH_SERVICE%" "%GAMESTASH_PORT%" || goto :SetupFail
    if /I "%DEBTMANAGER_DEPLOY%"=="true" call :DeployProject "%API_PATH%\CodeName399.DebtManager.API\CodeName399.DebtManager.API.csproj" "%DEBTMANAGER_PATH%" "%DEBTMANAGER_SERVICE%" "%DEBTMANAGER_PORT%" || goto :SetupFail
    if /I "%KITE_DEPLOY%"=="true" call :DeployProject "%API_PATH%\CodeName399.Kite.API\CodeName399.Kite.API.csproj" "%KITE_PATH%" "%KITE_SERVICE%" "%KITE_PORT%" || goto :SetupFail
) else (
...
if errorlevel 1 echo [WARNING] Nginx is not listening on TCP %NGINX_PORT%.
...
REM ============================================================================
REM SUBROUTINES - ALL ARE AFTER THE MAIN FLOW.
REM This is intentional. No main-flow execution can fall into these labels.
REM ============================================================================
...
sc.exe query "%CHECK_SERVICE%" | findstr /I "RUNNING" >nul
if errorlevel 1 (
...
for /L %%N in (1,1,20) do (
    ...
    if not errorlevel 1 (
        set "CHECK_FOUND=1"
        goto :EnsureServicePortDone
    )
    timeout /t 1 /nobreak >nul
)

:EnsureServicePortDone
...
if not exist "%DEPLOY_PATH%" mkdir "%DEPLOY_PATH%" >nul 2>&1
dotnet publish "%DEPLOY_CSPROJ%" -c Release -o "%DEPLOY_PATH%" --no-restore
if errorlevel 1 exit /b 1

call :EnsureWindowsService "%DEPLOY_SERVICE%" "%DEPLOY_PATH%" "%DEPLOY_PORT%"
if errorlevel 1 exit /b 1
...
if errorlevel 1 (
    sc.exe create "%SVC_NAME%" binPath= "\"%SVC_EXE%\"" start= auto DisplayName= "%SVC_NAME%"
    if errorlevel 1 exit /b 1
)
...
if not defined UI_SOURCE (
    for /d %%D in ("%ANGULAR_PATH%\dist\*") do (
        if exist "%%~D\browser\index.html" set "UI_SOURCE=%%~D\browser"
    )
)
...
if not exist "%NGINX_EXE%" call :InstallNginxFiles
...
if not exist "%NGINX_ROOT%\conf\mime.types" (
    echo [ERROR] Nginx mime.types missing. Reinstalling Nginx files...
    call :InstallNginxFiles
)
if not exist "%NGINX_ROOT%\conf\mime.types" (
    echo [ERROR] Nginx mime.types still missing.
    exit /b 1
)
...
call :CheckPort 80
if errorlevel 1 (
    echo [ERROR] Nginx is not listening on TCP 80.
    if exist "%NGINX_ROOT%\logs\error.log" type "%NGINX_ROOT%\logs\error.log"
    exit /b 1
)
...
echo        Testing Gateway directly on 127.0.0.1:5000...
curl.exe --noproxy "*" --connect-timeout 3 --max-time 5 -sS -D "%TEMP%\CodeName399-gateway-headers.txt" -o "%TEMP%\CodeName399-gateway-body.txt" http://127.0.0.1:5000/ >nul 2>"%TEMP%\CodeName399-gateway-error.txt"
if errorlevel 1 (
    echo [ERROR] Gateway did not accept an HTTP connection.
    type "%TEMP%\CodeName399-gateway-error.txt"
    exit /b 1
)
echo [OK] Gateway is responding on port 5000.
...
findstr /I /C:"X-CodeName399-Route: gateway" "%TEMP%\CodeName399-api-headers.txt" >nul
if errorlevel 1 (
...
echo        Writing rotated Cloudflare tunnel token...
...
REM Remove any previous cloudflared service definition.
sc.exe stop cloudflared >nul 2>&1
timeout /t 2 /nobreak >nul
sc.exe delete cloudflared >nul 2>&1
timeout /t 2 /nobreak >nul

echo        Creating cloudflared Windows service...
...
for /L %%N in (1,1,15) do (
    ...
    if not errorlevel 1 (
        set "CF_RUNNING=1"
        goto :CloudflareServiceDone
    )
    timeout /t 1 /nobreak >nul
)

:CloudflareServiceDone
